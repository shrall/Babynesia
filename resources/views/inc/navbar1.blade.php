...
<div class="transition-all duration-300 menulist xl:hidden sm:w-3/4 w-5/6 h-screen fixed -left-full top-0" style="z-index: 5002">
    ...
    <div class="bg-white pt-4 pb-28 px-4 h-full overflow-y-scroll">
        ...
        <div class="my-3">
            <form action="{{ route('user.user.index') }}" id="formchecker"></form>
            <input type="hidden" name="checker" value="profile" form="formchecker">
            <button type="submit" form="formchecker" class="font-encode-sans font-bold {{ !empty($page)&&$page == 'profile' ? 'text-'.$color[1].'-500' : 'text-slate-900' }} hover:text-{{ $color[1] }}-500">
                Account
            </button>
        </div>
        <hr>
