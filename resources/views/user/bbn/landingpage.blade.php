        ...
        <div>
            <div class="mb-6 flex flex-wrap gap-3 justify-center">
                @foreach ($hotdeals as $hd)
                    <img src="{{ 'https://tokobayifiv.com/public/uploads/' . $hd->image }}"
                        class="w-100 h-vh-40 object-cover" alt="">
                @endforeach
            </div>
        </div>
                ...
                <div class="bg-white rounded-b-md shadow-sm px-3 py-3">
                    <ul class="mt-3">
                        ...
                        @if (config('services.app.type') != 1)
                            <li class="my-1">
                                <form action="{{ route('list_products') }}">
                                    <input type="hidden" value="fav" name="filterproduct">
                                    <input type="hidden" name="pagehighlight" value="FAV"">
                                    <button type="submit"
                                        class="text-sm font-bold hover:text-{{ $color[1] }}-500 {{ $filteredproduct == 'fav' ? 'text-' . $color[1] . '-500' : 'text-slate-900' }} font-encode-sans">
                                        FAV Product
                                    </button>
                                </form>
                            </li>
                            ...
                            <a href="{{ route('produk.show', $produk) }}" class="shadow-sm bg-white">
                                ...
                                <div class="p-3 pb-5">
                                    <h6 class="font-encode-sans font-bold sm:text-base text-sm text-clip">
                                        ...
                                        {{ $produk->nama_produk }} {{ $produk->app_type == 1 ? '' : ' - FAV' }}
                                    </h6>
                                    <div class="flex justify-between items-center sm:my-3 my-2">

                                        <h2
                                            class="font-concert-one {{ $produk->countStock($produk) != 0 ? 'text-slate-900' : 'text-gray-400' }} xl:text-2xl text-xl">
                                            Rp.
