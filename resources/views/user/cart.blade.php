...
<div class="container mx-auto xl:px-32 px-3 xl:pt-6 xl:pb-10 pt-3 pb-5">
    ...
    <div class="w-full bg-white rounded-md shadow-sm mt-3 px-3 pt-3 pb-5">
        ...
        <form action="{{ route('user.detailcart.update', $cart) }}" method="post" id="updatecart{{ $cart->no_detail_cart }}"
            enctype="multipart/form-data">@csrf
            @method('PATCH')</form>
        <input type="hidden" name="idcart" value="{{ $cart->no_detail_cart }}" id="idcart" form="updatecart{{ $cart->no_detail_cart }}">
        ...
        <div class="hidden sm:block border-l-2 border-gray-100 pl-4">
            <div class="flex items-center">
                <p class="font-encode-sans text-sm text-slate-900 sm:text-base">
                    <input type="number" form="updatecart{{ $cart->no_detail_cart }}" value="{{ $cart->jumlah }}" name="jumlah1"
    ...
    <div class="mt-3 flex justify-between">
        <div class="flex items-center">
            <p class="font-encode-sans text-sm text-slate-900 sm:text-base">
                <input type="number" form="updatecart{{ $cart->no_detail_cart }}" value="{{ $cart->jumlah }}" name="jumlah"
                    class="bg-neutral-100 border font-encode-sans w-7 text-center rounded-sm"
                    id="inputnumber{{ $cart->jumlah }}"> x
                Rp.
                {{ $cart->produk->stat == 'd' ? substr(number_format($cart->produk->harga_sale,2,",","."), 0, -3) : substr(number_format($cart->produk->harga,2,",","."), 0, -3) }}
...
<form action="{{ route('user.receiver.create') }}" method="get">
    ...
    <div class="xl:flex xl:items-center">
        <div class="w-full bg-white rounded-md shadow-sm mt-3 px-3 py-5 xl:w-8/10">
            ...
            <div>
                <label for="catatan-tambahan" class="font-encode-sans font-bold text-slate-900 text-sm sm:text-base">
                    Catatan Tambahan
                </label>
            </div>
            ...
            <div class="xl:hidden">
                ...
                <div class="text-center mt-1">
                    <a href="{{ route('list_products') }}" class="font-encode-sans text-gray-400 text-sm sm:text-base">
                        Continue Shopping
                    </a>
                </div>
            </div>
