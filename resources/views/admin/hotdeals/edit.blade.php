...
@section('content')
    ...
    <form action="{{ route('adminpage.hotdeals.update', $hotdeals->id) }}" method="post" enctype="multipart/form-data">
        ...
        <div class="admin-card">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <div class="flex items-center gap-2">
                            <input type="radio" name="type" value="Video"
                                {{ $hotdeals->type == 'Video' ? 'checked' : null }} id="radio-2">
            ...
            <div class="col-span-12 type-Video  {{$hotdeals->type != 'Video' ? 'hidden' : 'block'}}">
                <div class="flex flex-col gap-2">
            ...
            <div class="col-span-12 type-Gambar {{$hotdeals->type != 'Gambar' ? 'hidden' : 'block'}}">
                <div class="flex flex-col gap-2">
                    ...
                    <div class="flex items-end">
                        <img @if ($hotdeals->image) src="{{ asset('uploads/' . $hotdeals->image) }}" @else src="{{ asset('svg/images.svg') }}" @endif
            ...
            <div class="col-span-12 status-2 {{$hotdeals->status != '2' ? 'hidden' : 'block'}}">
                <div class="flex flex-col gap-2">
                    <label for="from_date" class="text-xl">Tampilkan dari tanggal:</label>
                    <input type="date" name="from_date" id="from_date" class="admin-input" value="{{$hotdeals->from_date}}">
                </div>
            </div>
            <div class="col-span-12 status-2 {{$hotdeals->status != '2' ? 'hidden' : 'block'}}">
                <div class="flex flex-col gap-2">
                    <label for="until_date" class="text-xl">Sampai dengan tanggal:</label>
                    <input type="date" name="until_date" id="until_date" class="admin-input" value="{{$hotdeals->until_date}}">
                </div>
            </div>
            <div class="col-span-12">
