                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <a onclick="openModal('delete-{{$news->no}}')"
            ...
            <div class="rounded-lg bg-white px-8 pt-8 pb-6 absolute flex flex-col gap-y-4 w-128">
                ...
                <div class="flex items-center justify-center px-8 py-4">
                    <div class="flex flex-col gap-y-2 text-center">
                        <span>Apakah kamu yakin ingin menghapus data dengan nama {{ $news->judul }}?</span>
                    </div>
                </div>
                <div class="admin-action-button-danger w-full cursor-pointer"
                    onclick="event.preventDefault(); document.getElementById('delete-news-form-{{ $news->no }}').submit();">
                    Hapus
                    <span class=" fa fa-fw fa-trash-alt ml-2"></span>
                </div>
                <form action="{{ route('adminpage.news.destroy', $news->no) }}"
                    id="delete-news-form-{{ $news->no }}" method="post">
                    @csrf
