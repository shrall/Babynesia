...
@section('content')
    ...
    <form action="{{ route('adminpage.faq.update', $faq->no_faq) }}" method="post">
        @csrf
        @method('PATCH')
        <div class="admin-card mb-2">
            <div class="col-span-3">Pertanyaan</div>
            <div class="col-span-9 flex items-center gap-x-2">
                <input type="text" name="pertanyaan" id="pertanyaan" class="admin-input" value="{{$faq->pertanyaan}}" required>
            </div>
            <div class="col-span-3">Jawaban</div>
            <div class="col-span-9 flex items-center gap-x-2">
                <input type="text" name="jawaban" id="jawaban" class="admin-input" value="{{$faq->jawaban}}" required>
            </div>
