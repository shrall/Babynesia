            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <div class="p-2 bg-{{ $color[0] }}-300 text-sm font-bold font-encode-sans mx-2 rounded-md text-white">{{ $page }}</div>
                    @else
                        <a href="{{ $url }}" class="text-sm sm:text-base hover:text-{{ $color[1] }}-500 text-slate-900 mx-2 sm:mx-3">{{ $page }}</a>
                    @endif
                @endforeach
            @endif
        @endforeach
